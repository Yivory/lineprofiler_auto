...
# Embedded file name: controllers\micro_services.py
...
def mic_project_generator(path, i):
    ...
    tifffile.imwrite(path + '\\mic_project' + str(i) + '.tif', mic_project.astype(np.uint16))
    return out
...
def image_writer(image, profiles, path):
    tifffile.imwrite((path + '\\Image_with_profiles.tif'), (np.asarray(profiles[..., 0:3]).astype(np.uint16)),
      photometric='rgb')
    image = image.astype(np.uint32) * 10
    image += profiles
    image = np.clip(image, 0, 65535)
    tifffile.imwrite((path + '\\Image_overlay.tif'), (image[..., 0:3].astype(np.uint16)), photometric='rgb')
# okay decompiling controllers.micro_services.pyc
