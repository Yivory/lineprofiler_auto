...
# Embedded file name: controllers\fit_function_factory.py
import numpy as np
...
@register
class bigaussian:
    fit_parameters = ('Intensity 1: ', 'Width 1: ', 'Center 1: ', 'Intensity 2: ',
    ...
    def fit(x, h1, w1, c1, h2, w2, c2, noise_lvl):
        ...
        gaussian: ndarray
            (nx1) array of y values corresponding to the given parameters
        """
        return gaussian.fit(x, h1, w1, c1, 0) + gaussian.fit(x, h2, w2, c2, 0) + noise_lvl


@register
class trigaussian:
    fit_parameters = ('Intensity 1: ', 'Width 1: ', 'Center 1: ', 'Intensity 2: ',
                      'Width 2: ', 'Center 2: ', 'Intensity 3: ', 'Width 3: ', 'Center 3: ',
                      'Offset: ')
    ...
    def fit(x, h1, w1, c1, h2, w2, c2, h3, w3, c3, noise_lvl):
        ...
        gaussian: ndarray
            (nx1) array of y values corresponding to the given parameters
        """
        return gaussian.fit(x, h1, w1, c1, 0) + gaussian.fit(x, h2, w2, c2, 0) + gaussian.fit(x, h3, w3, c3, 0) + noise_lvl


@register
class cylinder_projection:
    fit_parameters = ('Intensity cylinder: ', 'Center: ', 'Inner Radius: ', 'Outer Radius: ',
                      'Offset: ', 'Blur: ')
    ...
    def guess(param):
        ...
        return [2 * param['height'], param['CM'], 25 * r_multi + 8.75, 25 * r_multi + 17.5, 0, 10]
...
class multi_cylinder_projection:
    ...
    def fit(x, i1, i2, i3, c, expansion, offset, blur):
        cyl1 = cylinder_projection.fit(x, i1, c, 25 * expansion / 2 - 17.5, 25 * expansion / 2 - 8.75, 0, blur)
        cyl2 = cylinder_projection.fit(x, i2, c, 42.5 * expansion / 2, 42.5 * expansion / 2 + 8.75, 0, blur)
        cyl3 = cylinder_projection.fit(x, i3, c, 25 * expansion / 2 + 8.75, 25 * expansion / 2 + 17.5, 0, blur)
        return cyl1 + cyl2 + cyl3 + offset
# okay decompiling controllers.fit_function_factory.pyc
