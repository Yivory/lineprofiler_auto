...
# Embedded file name: controllers\processing_SNC.py
from utility import *
    ...
    def _show_profiles(self):
        ...
        for i in range(len(self.shapes)):
                    ...
                    if distance > self.upper_limit:
                        ...
                        if profile.shape[0] < int(current_profile_width):
                            ...
                            if profile_g.shape[0] != current_profile_width:
                                ...
                                if self.three_channel:
                                    profile_b = line_profile((self.current_image[2]), (line['start']), (line['end']), px_size=(self.px_size), sampling=(self.sampling))
            ...
            if current_profile['red']:
                ...
                self.sig_plot_data.emit(red_mean, current_profile_width, i, self.path, color, len(current_profile['red']))
                self.results['p_green'] += current_profile['green']
                green_mean = self.save_avg_profile(current_profile['green'], 'green_' + str(i))
                self.sig_plot_data.emit(green_mean, current_profile_width, i, self.path + '/green', color, len(current_profile['green']))
                if self.three_channel:
                    self.results['p_blue'] += current_profile['blue']
                    blue_mean = self.save_avg_profile(current_profile['blue'], 'blue_' + str(i))
                    self.sig_plot_data.emit(blue_mean, current_profile_width, i, self.path + '/blue', color, len(current_profile['blue']))
        ...
        self.sig_plot_data.emit(red_mean, current_profile_width, 9999, self.path, (1.0,
                                                                                   0.0,
                                                                                   0.0,
...
# okay decompiling controllers.processing_SNC.pyc
